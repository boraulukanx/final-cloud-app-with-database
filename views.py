...
def submit(request, course_id):
    ...
    # If not a POST request or user is not enrolled, handle appropriately
...
def show_exam_result(request, course_id, submission_id):
    ...
    for question in course.question_set.all():
        ...
        selected_correct_choices = set(selected_choice_ids).intersection(correct_choice_ids)
        
        is_correct = len(selected_correct_choices) == len(correct_choice_ids)
    ...
    # Calculate the scaled total score as a percentage
    scaled_total_score = round((total_score / max_score) * 100, 1)
    
    context = {
        'course': course,
        'submission': submission,
        'total_score': int(scaled_total_score),
        'max_score': max_score,
